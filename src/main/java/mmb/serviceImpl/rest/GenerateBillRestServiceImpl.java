...
@Service
@Transactional
public class GenerateBillRestServiceImpl implements GenerateBillRestService {
	...
	@Autowired
	public GenerateBillRestServiceImpl(GenerateBillRepo generateBillRepository, RawMaterialRepo rawMaterialRepo,
...
//	}
	...
	public List<GenerateBillDTO> getAllBills() {
	    List<GenerateBill> bills = generateBillRepo.findAll();
...
//	}
	...
	public GenerateBillDTO saveOrUpdate(GenerateBillDTO dto) {
	    ...
	    // --- update materialsBill ---
	    if (bill.getMaterialsBill() == null) {
	        bill.setMaterialsBill(new ArrayList<>());
	    } else {
	        bill.getMaterialsBill().clear();
	    }
	    ...
	    // --- update otherWorks ---
	    if (bill.getOtherWorks() == null) {
	        bill.setOtherWorks(new ArrayList<>());
	    } else {
	        bill.getOtherWorks().clear();
	    }
	    ...
	    if (dto.getOtherWorks() != null) {
	        for (OtherWorkDTO owDto : dto.getOtherWorks()) {
	            ...
	            double otherWorkPrice = 0.0;
	            if (owDto.getPrice() != null) {
	                otherWorkPrice = owDto.getPrice();
	            }
	            totalOtherWorkAmt += otherWorkPrice;
	    ...
	    // --- save entity ---
	    GenerateBill savedBill = generateBillRepo.save(bill);

	    // --- convert entity back to DTO ---
	    GenerateBillDTO resultDto = new GenerateBillDTO();
	    resultDto.setBillId(savedBill.getBillId());
	    resultDto.setCustomerName(savedBill.getCustomerName());
	    resultDto.setMobileNo(savedBill.getMobileNo());
	    resultDto.setWorkAddress(savedBill.getWorkAddress());
	    resultDto.setWorkDate(savedBill.getWorkDate());
	    resultDto.setBoringType(savedBill.getBoringType());
	    resultDto.setBoringDia(savedBill.getBoringDia());
	    resultDto.setPriceQntDtls(savedBill.getPriceQntDtls());
	    resultDto.setDrillingPrice(savedBill.getDrillingPrice());
	    resultDto.setTransportingVehicleType(savedBill.getTransportingVehicleType());
	    resultDto.setTransportingPrice(savedBill.getTransportingPrice());
	    resultDto.setTotalDrilling(savedBill.getTotalDrilling());
	    resultDto.setTotalAdvance(savedBill.getTotalAdvance());
	    resultDto.setTotalDrillingAmt(savedBill.getTotalDrillingAmt());
	    resultDto.setTotalMatrialAmt(savedBill.getTotalMatrialAmt());
	    resultDto.setTotalOthWorkAmt(savedBill.getTotalOthWorkAmt());
	    // map collections if needed...
